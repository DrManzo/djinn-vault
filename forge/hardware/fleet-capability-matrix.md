...
- Max bed: ~100°C
- Enclosure: No
- **Critical known electrical issue:** nozzle_mcu UART cable harness (BUG-014) — recurring key561 dropouts (bytes_invalid growing = intermittent partial-wire contact). History: originally fixed 2026-06-03 (reseat), recurred 2026-06-28, fully diagnosed 2026-07-05 as degraded cable, replacement cable installed 2026-07-09. **Recurred again 2026-07-12** on camood-v2 PETG jobs — a 4-copy plate crashed mid-print (21 min in), and a single-copy job that printed cleanly for the full 3h37m then crashed 6 seconds after completion during the END_PRINT/park move (identical signature to the original 6/28 park-position crash). Part quality unaffected both times (full/correct filament deposited). **Recurred again 2026-07-13**, still on camood-v2, one-copy-at-a-time rule already in effect: 2 mid-print errors (operator-recovered both times, print completed), then a third error post-completion during the END_PRINT park move — same park-move signature as 2026-07-12's single-copy incident. **Cable physically replaced again, 2026-07-13** (second replacement — first was 2026-07-09, lasted only 3 days before recurring). Not otherwise re-diagnosed — this is a hardware swap only, per Javier's original "leave it for now" call. **Verification pending** — no test print run yet since this replacement; given how fast the 07-09 replacement also recurred, don't treat the swap alone as confirmation. See `djinn/logs/bugs.md` BUG-014 for full history.
- **Operating rule (2026-07-12, per Javier, still in effect despite the 2026-07-13 cable replacement):** camood-v2 — print ONE COPY AT A TIME on Calliope. Do not lift this just because the cable was swapped again — wait for multiple confirmed-clean prints first.
- **Fan cap rule:** Hard cap at S128 (50%) system-wide — M106 S255 at bridge infill creates EMI spike → instant nozzle_mcu dropout. This is a hardware constraint, not a preference. **⚠️ Confirmed 2026-07-13: this cap was never actually enforced anywhere** — checked real Calliope-sliced gcode and found M106 running above S128 on 99.6–99.9% of commands for camood-v2 (peak S229) and even a single S255 hit on an unrelated model (mario-pipe treesupport). This is a slicer-profile gap affecting every Calliope job, not a camood-specific issue — camood is just exposed to it far more because its geometry needs cooling constantly. `forge/tools/djinn-gcode-fancap` now exists to clamp M106 in any gcode file before it reaches the queue; recommend running it on every Calliope job until the actual Creality Print profile is fixed at the source. See BUG-014 report for full detail — this may be the actual trigger behind the repeated recurrences, not (only) cable degradation.
