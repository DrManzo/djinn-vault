---
title: Sovereign — Canon
created: 2026-07-23
updated: 2026-07-23
tags: [personal, sovereign, canon, library]
---

# Canon

The library, subordinate to conduct. Per book: what it trains, what behavior it should change, which [[Protocols]] module it feeds. Full ownership/progress status lives in `personal/library/Book-Catalog.md` — this page doesn't duplicate that inventory, only maps the relevant subset to behavior.

Books tagged **[external strategy only]** are explicitly scoped away from close relationships per the Social Field split-rule in [[Protocols#Social-Field]].

## Shelf 1 — Power & Strategy

**48 Laws of Power** (Greene) — *[external strategy only]*
- Trains: reading power dynamics, concealment, timing.
- Changes: use concealment and restraint in strategic/external contexts only — never as the default mode with people you're actually close to.
- Feeds: Reputation & Taste, Work (positioning).

**The Prince** (Machiavelli) — *[external strategy only]*
- Trains: stability through consistency of conduct over popularity.
- Changes: stop optimizing for being liked; optimize for being predictable and respected in your own standards.
- Feeds: Command, Reputation & Taste.

**The Art of Worldly Wisdom** (Gracián) — *[external strategy only]*
- Trains: measured social calibration, patience, discretion.
- Changes: slow the response in external/strategic settings; don't perform availability.
- Feeds: Social Field (external contexts only).

**The Art of War** (Sun Tzu) — *[external strategy only]*
- Trains: positioning before confrontation, knowing when not to engage.
- Changes: choose Work/Reputation moves deliberately instead of reactively.
- Feeds: Work & Selective Output.

## Shelf 2 — Stoicism & Discipline

**Meditations** (Marcus Aurelius)
- Trains: command over reaction, not over outcome or others' perception.
- Changes: the nightly command mark audits your own action, not whether others understood it.
- Feeds: Command.

**The Practicing Stoic** (Farnsworth)
- Trains: applied daily discipline structures, not just theory.
- Changes: convert Stoic reading directly into the actual morning/evening rhythm in Protocols — not more reading about discipline.
- Feeds: Command.

**Letters from a Stoic** (Seneca)
- Trains: long-view patience under real constraint (time, money, mortality).
- Changes: use for the weekly review's "what became theater" question — this is Seneca's whole project.
- Feeds: Command.

## Shelf 3 — Identity & Integration

**Beyond Order** (Peterson)
- Trains: order/chaos balance, taking on responsibility as an identity-building act.
- Changes: the non-negotiables in [[Home]] are the "order" — everything outside them is allowed to flex.
- Feeds: Home (identity frame), Command.

**Maps of Meaning** (Peterson)
- Trains: meaning through voluntary confrontation with what's being avoided.
- Changes: directly informs the edge-dwelling correction — approach what's being watched from a distance instead of narrating it.
- Feeds: Social Field, Work & Selective Output.

**The Red Book** / **Man and His Symbols** (Jung)
- Trains: shadow integration — the difference between suppressing an unwanted trait and integrating it.
- Changes: the identity-as-collection-of-parts framing — don't perform a cleaned-up self, integrate the actual one.
- Feeds: Home (identity frame).

**Boundaries of the Soul** (Singer)
- Trains: Jungian boundary formation, differentiation from others.
- Changes: directly supports the Social Field split-rule — differentiation in strategic contexts, permeability in close ones.
- Feeds: Social Field.

**No More Mr. Nice Guy** (Glover)
- Trains: naming approval-seeking and covert contracts.
- Changes: the self-disclosure floor exists because of exactly this pattern — say the actual preference instead of managing the room.
- Feeds: Social Field.

**Emotional Intelligence** (Goleman)
- Trains: naming and regulating internal state before acting on it.
- Changes: feeds the morning claim — name the state, don't let it run the day unnamed.
- Feeds: Command.

**Daring Greatly** (Brown)
- Trains: vulnerability as the entry cost of real connection, not a weakness.
- Changes: directly opposes edge-dwelling — engagement requires exposure, there's no version of connection without it.
- Feeds: Social Field.

## Shelf 4 — Execution & Systems

**How to Take Smart Notes** (Ahrens)
- Trains: externalizing thought into a system instead of carrying all of it in working memory.
- Changes: use for weekly review capture — write the rule down, don't just resolve to remember it.
- Feeds: Command.

---

## Acquisition queue

Not yet owned. Order reflects what actually unblocks Sovereign fastest, not raw appeal.

1. **Atomic Habits** (Clear) — trains identity-linked habit architecture and tiny-scale behavior design; gives Command's daily binary-tracking design its actual theoretical backing. → Command, Body.
2. **Mastery** (Greene) — trains long-horizon deliberate practice; supports the "one decisive move" discipline in Work without needing constant novelty. → Work & Selective Output.
3. **Discipline Is Destiny** (Holiday) — readable daily-discipline framing across body and mind; reinforces Body's "no negotiation" rule with a citable frame. → Body, Command.
4. **The Laws of Human Nature** (Greene) — *[external strategy only]* — trains reading others' patterns accurately; useful for Reputation/Work positioning, explicitly not for close relationships. → Reputation & Taste, Work (external contexts only).
5. **Attached** (Levine & Heller) — trains attachment-style recognition; gives the self-disclosure floor a concrete mechanism by naming what "unforced disclosure" is actually resisting. → Social Field.
6. **Man's Search for Meaning** (Frankl) — trains values-anchored action independent of circumstance; backs the "insight without a rule is indulgence" principle at its most serious register. → Home (core purpose), Command.
7. **The War of Art** (Pressfield) — trains naming resistance as a force to move through, not negotiate with; reinforces Work's "execute or degrade gracefully, never skip" rule. → Work & Selective Output.
8. **The Art of Seduction** (Greene) — *[external strategy only — hold]*. Of everything on this list, most likely to bleed strategic/concealment logic into close relationships, which is the exact pattern this doctrine exists to correct. Hold until the Social Field split-rule has been running cleanly for a few weeks. When it's read, tag it "external strategy only" the same as 48 Laws and The Laws of Human Nature.
