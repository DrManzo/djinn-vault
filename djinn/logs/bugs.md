...
| 2026-07-20 | Claude | Nemesis / stock FlashOS firmware | high | resolved (pending full-print confirmation) | Root cause confirmed: Nemesis's stock.cfg defines two print-start macros — `NOLEVELING_PRINT_FILE` (skips real leveling, computes an offset then discards it to `0.0`) and `LEVELING_PRINT_FILE` (runs a real `BED_MESH_CALIBRATE` and keeps it). Every failed attempt, including all of mine via Moonraker's native `/printer/print/start`, hit the no-leveling path by default. Javier started a print by hand with leveling enabled at file-select — full 17x17 mesh captured, no Z-offset reset in the log, webcam confirmed a clean first layer ~5min in. Fix for any future automation: call `LEVELING_PRINT_FILE FILENAME=<name>` via `/printer/gcode/script`, never the native print/start endpoint. No existing djinn script needed patching (none currently targets Nemesis). | [[2026-07-19_bug-nemesis-z-offset-recalibration-too-aggressive-nozzle-scraping]] |
