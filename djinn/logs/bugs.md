...
| 2026-08-24 | Claude | printer-files / Alexandria storage | high | fixed | `library/` and `review/` were completely missing from Alexandria's `printer-files/` tree (and from `~/printer-files/` on Salomon) — surfaced while tracking down the Tornado Recycler STL. No deletion event found in bugs.md/build-log.md history; most likely never fully migrated during the mid-July Library→Alexandria move (same window as the already-logged Backpack Boyz/Camood TTHQ losses). Real source was intact on Typhon (`C:\Forge\models\library\`, `C:\Forge\models\review\`) — pulled all 455 STLs as one verified tar (per Typhon file-transfer rules), filtered out OrcaSlicer's own bundled bed-model STLs and already-current `queue`/`utility` files, copied `library/` (1.1GB) + `review/` (493MB) into Alexandria purely additively, symlinked into `~/printer-files/`. Nothing deleted from Typhon; nothing overwritten on Alexandria. | [[2026-08-24_typhon-stl-library-recovery]] |
