...
| 2026-07-16 | Claude | Nemesis / OrcaSlicer machine profile | high | ⚠️ WAS WRONG, corrected 2026-07-20 | Originally diagnosed as "real firmware is corner-origin (0-220)" and fixed by setting printable_area to 0x0-220x220. Live Klipper verification 2026-07-20 (axis_minimum/maximum -125/125, mesh_min/max -105/105, SCREWS_TILT_CALCULATE probe coords) proved Nemesis is genuinely center-origin — the "fix" was itself the bug, silently mispositioning Salomon-sliced jobs since 7/16. Reverted to center-origin (-110/-110 to 110/110). | [[2026-07-16_bug-nemesis-orcaslicer-center-origin-printable-area]] |
...
| 2026-07-19 | Claude | Nemesis / Z-offset calibration | high | fixed (Klipper side) | First-pass paper-test landed at `probe.z_offset: -0.336`, too aggressive, caused scraping. Redone correctly 2026-07-20 (offset first, then fresh mesh): `-0.101`, verified via clean 0.537mm mesh. | [[2026-07-19_bug-nemesis-z-offset-recalibration-too-aggressive-nozzle-scraping]] |
| 2026-07-20 | Claude | Nemesis / stock FlashOS firmware | high | open — needs paper-check verification | Real cause of continued scraping suspected: a separate, closed-source FlashOS leveling dialog (port 8899, independent of Klipper's `probe.z_offset`) runs on every print-start regardless of entry point (touchscreen, Moonraker API, raw Klipper `SDCARD_PRINT_FILE` — all confirmed to hit it). It computes a fresh offset each time (~0.85-1.05mm, watched across attempts) but discards it and applies `0.0` right before printing. No remote bypass found after exhaustive testing, including a live-tested `mod_params.load_zoffset` override (confirmed dead — same storage slot, dialog writes last). Whether `0.0` is actually dangerous is UNCONFIRMED — no real print has run far enough under the current good calibration to prove it; next step is a non-destructive `G28`+`SET_GCODE_OFFSET Z=0`+`G1 Z0.25`+paper-drag check, not a real print or (Javier's proposal) re-leveling screws (screws fix tilt, not the uniform height this bug affects). | [[2026-07-19_bug-nemesis-z-offset-recalibration-too-aggressive-nozzle-scraping]] |
| 2026-07-20 | Claude | Nemesis / Moonraker (self-inflicted) | high | fixed | Attempting to restart a hung Moonraker (stuck behind a blocking gcode request) tore down a chroot bind-mount (`start-stop-daemon` unavailable in this busybox env). Manual reconstruction hit a hard wall — the venv requires Python 3.11 syntax the outer-namespace interpreter (actually 3.7.2) can't parse. Root cause: `forge-x` runs as a full chroot at `/data/.mod/.forge-x/`; Moonraker must run *inside* it, not be invoked from outer filesystem paths. Fixed by a full reboot, which correctly restored all boot-time mounts and relaunched Moonraker in its chroot (same PID as before, confirming deterministic init). Klipper itself was never affected — calibration data survived intact. | [[2026-07-19_bug-nemesis-z-offset-recalibration-too-aggressive-nozzle-scraping]] |
