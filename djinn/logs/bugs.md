...
| 2026-07-20 | Claude | Nemesis / stock FlashOS firmware | high | open — needs Javier physically present | Real cause of continued scraping found: a separate, closed-source FlashOS leveling dialog (port 8899, independent of Klipper's `probe.z_offset`) runs on every print-start regardless of entry point (touchscreen, Moonraker API, raw Klipper `SDCARD_PRINT_FILE` — all confirmed to hit it). It computes a fresh offset each time but discards it and applies `0.0` right before printing. Does not accept Klipper's `TESTZ`/`ACCEPT` gcode despite near-identical console text. No remote bypass found after exhaustive testing. Needs physical touchscreen interaction or a verified (not guessed) direct edit to its own `zLevelOffset` value in `Adventurer5M.json`. | [[2026-07-19_bug-nemesis-z-offset-recalibration-too-aggressive-nozzle-scraping]] |
