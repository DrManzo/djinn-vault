...
| 2026-08-01 | Claude | forge / Calliope / PRINT-PROFILES.md | medium | workaround applied, unconfirmed | Kraken pipe (Calliope, 2026-07-20 job) leaks air through general wall porosity, not a seam or joint. Source STL confirmed watertight/valid manifold (genus 23 — complex tentacle-wrap topology). Could not reach Calliope/Moonraker to confirm actual slice settings used. No existing Calliope profile tier (proto/standard/production — max 4 walls, flat 210°C hotend) targets airtightness; added a new `airtight` tier (5 walls, 215°C, 0.16mm layers, slower outer wall) to `forge/PRINT-PROFILES.md`. Not yet verified on a reprint. | [[2026-08-01_bug-kraken-pipe-wall-porosity-no-airtight-profile-tier]] |
| 2026-08-01 | Claude | djinn-model-slice (~/.local/bin, not git-tracked) | high | fixed | Entire automated shop-slicing pipeline non-functional since the 2026-07-12 department restructure — `SLICE_SH` still pointed at the pre-restructure `djinn/printer/forge-slicer/slice.sh` (real path is now `forge/forge-slicer/slice.sh`), and `PROFILE_DIR` pointed at `~/printer-files/creality-v3plus-profile/`, a directory that doesn't exist at all (dead leftover from a pre-Docker/OrcaSlicer-CLI architecture). Every invocation failed at the process-profile existence check regardless of job. Fixed both paths plus a subdirectory casing mismatch (`Processes/`→`process/`, `Materials/`→`filament/`). Found and fixed while slicing a validation test piece for TASK-107. | [[2026-08-01_bug-djinn-model-slice-dead-paths-since-department-restructure]] |
