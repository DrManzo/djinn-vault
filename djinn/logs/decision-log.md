...
## 2026-09-25 — Typhon rebuild: fresh Ubuntu Server as new command-center, not a nursed Windows install

**Decision:** Wipe Typhon and install Ubuntu 26.04.1 Server as the new Djinn command-center, taking over Salomon's automation stack, rather than continuing to complete Typhon's 2+-month-incomplete Windows onboarding. Salomon (much stronger hardware — 16-core Ryzen 9, 8GB RTX 5060, 29GB RAM) moves to Windows to run heavy CAD/Adobe/3D software instead.
**Why:** Javier's call — Typhon "hasn't been pulling its weight," and the existing Windows setup was never worth finishing. Rather than keep patching an install that was already a known-deviation from the original plan (see 2026-07-01 decision above — native Windows was itself a time-pressure workaround, not the intended architecture), start clean on the hardware better suited to being an always-on Linux service host, and put the GPU/CPU-heavy machine where GPU/CPU-heavy software actually needs to run.
**How to apply:** Full migration plan, inventory (27 timers + 18 services), and phase sequencing live in [[machines/salomon-typhon-role-swap-migration]]. Don't assume anything about old-Typhon's Windows state is worth preserving — this is a clean rebuild, not a migration of that install.

## 2026-09-25 — Matched Salomon's actual working Docker keyring file instead of following Docker's own install docs

**Decision:** `typhon-bootstrap.sh`'s Docker CE apt-keyring step was changed from `curl ... | sudo gpg --dearmor -o docker.asc` (Docker's documented method) to `curl ... | sudo tee docker.asc` — writing the raw ASCII-armored key with no dearmor step.
**Why:** the dearmored keyring was rejected by apt on new-Typhon's apt/gpg version as "unsupported filetype." Rather than debugging gpg/apt version interactions blind, diffed against Salomon's own already-working `/etc/apt/keyrings/docker.asc` byte-for-byte and found it was never dearmored in the first place, despite Docker's docs recommending it. Matched the known-working reference exactly.
**How to apply:** full bug report at [[2026-09-25_bug-docker-ce-apt-keyring-rejected-when-gpg-dearmored-per-official-docs]]. Don't trust a vendor's install docs over a live, working reference system when the two disagree and only one is confirmed correct.
