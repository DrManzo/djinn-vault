...
- trigger: manual
- created: 2026-06-01 by Claude (per Javier)
- context: Full 3D printing domain audit — 14 research areas covering everything Typhon's Forge needs to operate and grow. Hardware, materials, design pipeline, quality, scaling, business.

**Brief:**

Typhon's Forge runs a fully automated 3D print commission shop on a single Ender-3 V3 Plus (Calliope). The Djinn pipeline handles intake → quoting → slicing → printing → monitoring → shipping. Core system is built. What's missing is deep domain knowledge across every layer — hardware, materials, design toolchain, quality control, failure detection, scaling, and business. This research will directly inform the next 6 months of builds.

**What Djinn already has — do not re-research:**
- Calliope: Ender-3 V3 Plus, Klipper + Moonraker at 192.168.1.114
- Slicer: OrcaSlicer 2.3.2 (CLI-driven)
- Materials in use: PLA (standard), PETG (smoking accessories, enforced)
- Printer profile: 3 tiers (proto 0.28mm, standard 0.20mm, production 0.20mm)
- 6-agent manufacturing stack: DesignGen → DesignEdit → ProtoOpt → DOEPrintOpt → PlateNest → FairPrint
- Library: Puffco proxy accessories, decorative vases, GoPro/mic mounts, challenge coins
- Pipeline: confirm-gated, no autonomous prints, Javier owns orientation
- Business model: commission shop, 30% minimum margin, 35% smoking accessory upcharge

---

### Section 1 — Ender-3 V3 Plus: Unlocking Full Capability

The printer is running stock but most of its capability is untapped. Research what upgrades, Klipper macros, and calibration routines give the biggest return on print quality and reliability.

**Research questions:**
- What are the highest-impact Klipper macros for the Ender-3 V3 Plus specifically? (PRINT_START/END macros, adaptive meshing, purge line placement, filament change macros — what should every E3V3+ Klipper config include?)
- Input shaping (resonance compensation) — how to calibrate it on Ender-3 V3 Plus, what accelerometer to use (ADXL345 vs built-in), what speed improvements to expect, and whether it affects print quality at production profiles
- Pressure advance calibration — current best method, how often to recalibrate per material change, and whether to do it per-filament-brand or per-material-type
- Bed mesh leveling: adaptive mesh vs full mesh on the 300×300 bed — what's the right mesh density, when to trigger re-mesh (temperature changes, new filament)
- The Ender-3 V3 Plus has a direct drive extruder and auto-leveling. What maintenance schedule is recommended (nozzle replacement frequency, belt tension checks, PTFE tube inspection) for a commission shop running 40+ hours/week?
- What first-layer calibration approach is best for a production environment? Live Z offset, probe offset, or a dedicated calibration print before every session?
- Are there known firmware bugs or issues with the V3 Plus in Klipper that affect reliability? Any community-documented workarounds?

---

### Section 2 — PETG Mastery: The Primary Commission Material

Smoking accessories are PETG-enforced. This is the shop's primary product category. PETG is notoriously tricky — stringing, adhesion, warping on large flat pieces.

**Research questions:**
- Optimal PETG settings for Ender-3 V3 Plus specifically: temperature range (225–245°C nozzle), bed temp (70–85°C), cooling fan (how much? PETG hates cooling but needs some), retraction (direct drive: 0.5–2mm range), print speed for production quality
- PETG stringing: root causes and concrete fixes. Retraction, temperature, travel speed, z-hop — what actually eliminates stringing for functional parts where it matters?
- First layer adhesion for PETG: PEI spring steel vs glass vs garolite. The V3 Plus comes with PEI — is it sufficient for PETG? Any surface prep needed (IPA wipe, temperature tricks)?
- PETG for heat-exposed functional parts (dab accessories, anything near a flame): what is the actual heat deflection temperature (HDT) of PETG? Is it sufficient for a dab rig attachment? What temperature range makes it unsafe and what alternative materials handle higher temps?
- PETG vs PETG-CF (carbon fiber filled): is CF-PETG worth it for Puffco accessories? Strength difference, printability difference, cost difference, and whether a standard 0.4mm brass nozzle can handle CF-PETG
- Any PETG brands with known reliability issues or exceptional quality for the Ender-3 V3 Plus? Community consensus on specific filament brands for functional print shop use.
- PETG shelf life and storage: how much does humidity affect PETG vs PLA? What's the drying protocol (temperature, time) and how does wet PETG manifest in print quality?

---

### Section 3 — Materials Expansion: What Comes After PLA/PETG

The shop currently runs two materials. What should the next 2–3 additions be, and what infrastructure is required?

**Research questions:**
- **TPU (flexible):** What are the applications for a print shop (phone cases, gaskets, grips, mouthpieces)? Can the Ender-3 V3 Plus direct drive handle TPU well, or does it need bowden distance reduction? Shore hardness guide — what's appropriate for which use cases?
- **ASA:** Heat resistance, UV stability, outdoor durability vs PETG and ABS. For a maker selling outdoor/functional parts, when does ASA make sense? Enclosure requirements? Fume issues?
- **ABS:** Acetone smoothing is a major post-processing capability. For functional parts where surface finish matters (Puffco accessories, display pieces), is the ABS pipeline (enclosure + acetone vapor) worth building? What's the infrastructure cost?
- **Nylon (PA6/PA12):** Bearing-grade, wear-resistant. Any print shop applications? What infrastructure does it require (all-metal hotend, enclosure, drying)?
- **Resin (MSLA/SLA):** Should Typhon's Forge add a resin printer? What does it unlock that FDM can't do (fine detail, smooth surface finish, jewelry-grade parts)? Best entry-level MSLA for a production shop (Bambu Photon M5S, Elegoo Saturn series, Phrozen) — cost vs capability. Pipeline impact: resin requires post-processing (IPA wash, UV cure station), waste handling (toxic resin), and a completely separate workflow. Is it worth it for the Puffco niche specifically?
- **Material compatibility matrix:** For a shop offering multiple materials, how do you handle the slicer profile complexity, purge towers, and customer material selection? What's the recommended way to present material options to customers without overwhelming them?

---

### Section 4 — AI-Driven Quality Control: Failure Detection

The print monitor watches Moonraker for progress. It doesn't watch the actual print. A catastrophic failure (spaghetti, first-layer adhesion fail, warping) goes undetected until the operator checks manually.

**Research questions:**
- **Obico (formerly The Spaghetti Detective):** Current state in 2026. Self-hosted vs cloud. Accuracy of AI failure detection (false positive rate, what failure types it catches vs misses). Integration with Klipper/Moonraker — does it use the existing webcam? What webhook can it fire to alert via Telegram?
- **Webcam setup for Calliope:** What camera position, resolution, and angle gives Obico the best detection accuracy? Fixed mount vs adjustable?
- **Bambu AI vs Obico:** Bambu's onboard AI monitoring — is there a way to get equivalent intelligence without buying a Bambu printer?
- **First-layer inspection automation:** Tools that analyze the first layer (either via camera or via Klipper's flow monitoring) and pause if adhesion looks wrong. Current state of this in 2026.
- **Alternative approaches:** Vibration/current sensing as a failure proxy. Klipper has `[filament_switch_sensor]` and runout detection — are there community plugins that extend this to broader failure detection?
- **What to do when failure is detected:** Best practice for safe parking, filament retraction, and state preservation when a print fails mid-job. What Klipper macros handle this cleanly?

---

### Section 5 — Post-Processing: From Raw Print to Finished Product

Currently prints come off the bed and ship. What post-processing is worth building into the pipeline for a premium product?

**Research questions:**
- **Support removal:** Best techniques for clean support removal on PETG parts with complex geometry (Puffco attachments, recycler geometry). Tools (flush cutters, dental picks, heat gun), and whether soluble supports (PVA, HIPS) are worth the multi-material complexity
- **Sanding and surface finishing:** Workflow for sanding FDM prints to a smooth finish — grit progression, wet sanding, when it's worth doing vs not. For display pieces (vases, decorative items) vs functional parts (Puffco attachments).
- **ABS acetone smoothing:** Full workflow — vapor chamber setup, IPA wash station, safety considerations (ventilation, flammable vapor). What surface finish does it achieve and how does it affect dimensional accuracy?
- **Resin coating for PLA/PETG:** XTC-3D and similar brush-on coatings — do they actually improve surface finish enough to justify the cost and time? Any better alternatives in 2026?
- **UV resin dip coating:** Thin UV resin coat on FDM parts for a glossy finish — is this a real workflow used in production shops?
- **Painting and finishing:** Primer selection for PLA vs PETG (they have different surface energies), spray vs brush, recommended workflow for a small batch production environment
- **Branded packaging:** What do successful small Etsy 3D print shops use for packaging? Kraft boxes, poly mailers, tissue paper, custom stickers? What's the minimum viable branded unboxing experience at scale?

---

### Section 6 — The DOEPrintOpt Agent: Making It Real

The DOEPrintOpt agent is specced but not fully built. It uses Taguchi methods to find optimal slicer settings without test prints.

**Research questions:**
- **Taguchi method for 3D printing:** What are the correct control factors and levels for an L9 or L18 orthogonal array for FDM optimization? Standard factors: layer height, infill density, print speed, temperature, cooling — what else matters? What response variables are measured (tensile strength, dimensional accuracy, print time, surface roughness)?
- **Existing published Taguchi studies for FDM/PLA/PETG:** Are there peer-reviewed results showing optimal factor combinations for PLA and PETG on a Cartesian FDM printer? Can we use published results to skip the physical experiment entirely?
- **Virtual DOE approach:** Can slicer output data (estimated time, layer count, extrusion volume) serve as a proxy for physical test results in a Taguchi analysis? Has anyone in the maker community done this?
- **Simulation-based optimization:** Tools like SimScale or Netfabb have structural simulation — is it feasible to use FEA simulation results instead of physical tests for DOEPrintOpt? What open-source FEA tools work on Linux?
- **Tolerance stack-up:** For functional parts (threads, press fits, snap fits), what tolerance offset in OrcaSlicer compensates for FDM dimensional shrinkage in PLA vs PETG? Published calibration data?
- **Community calibration tools:** What calibration print libraries (Teaching Tech, Ellis's Print Tuning Guide) are considered authoritative in 2026? Can their results be encoded as a lookup table in DOEPrintOpt?

---

### Section 7 — Plate Nesting and Batch Optimization

PlateNestAgent currently does basic spatial arrangement. Research best-in-class algorithms.

**Research questions:**
- **2D bin packing algorithms for FDM plate nesting:** What are the current best algorithms? (Guillotine cut, Bottom-Left Fill, Genetic Algorithm, Deepnest-style irregular polygon packing). Which is most practical for 3D printing where models have variable bounding boxes?
- **Deepnest:** Current state in 2026. Is the web app still maintained? Is there a CLI/Python API? Can it handle STL bounding box data without importing the actual mesh?
- **Rotation for packing vs rotation for print quality:** Parts should be rotated for maximum packing density, but orientation affects print quality and strength. How do production shops resolve this conflict? Is there a standard approach?
- **Multi-material plate batching:** When a plate has parts in different materials, how do shops handle it? Sequential printing? AMS? What's realistic on an Ender-3 V3 Plus?
- **Plate clearance and part spacing:** Minimum clearance between parts for easy separation, brim clearance, skirt settings when multiple parts are on one plate. Community standards?
- **Plate preparation for different materials:** Does PEI surface treatment change per-material when running a mixed-material plate? Any practical considerations for commercial plate batching?

---

### Section 8 — DesignGen Pipeline: CAD Toolchain for AI-Generated Parts

DesignGenAgent generates parametric designs via OpenSCAD. Research whether this is the right toolchain.

**Research questions:**
- **OpenSCAD in 2026:** Is OpenSCAD still the best choice for AI-generated parametric CAD? What are its limitations for complex organic geometry (Puffco attachments, recycler water paths)? Is there a maintained fork or successor?
- **FreeCAD Scripting (Python API):** Can FreeCAD be driven entirely via Python API to generate parametric parts? How does the Python API compare to OpenSCAD for AI-generated geometry? Is it more capable for complex organic forms?
- **CadQuery:** Python library for CAD — state in 2026, what it can do that OpenSCAD can't, any limitations, community support
- **Build123d:** Newer Python CAD library, apparently more Pythonic than CadQuery. Current state, comparison with CadQuery, suitability for AI-driven parametric design
- **LLM + CAD performance in 2026:** What are the published benchmarks or community findings on LLM accuracy for generating OpenSCAD, CadQuery, and Build123d code? Which CAD DSL are LLMs most accurate with? Any fine-tuned models specifically for 3D modeling?
- **Water-tight mesh validation:** For Puffco accessories with internal water paths (recyclers), how do you validate that the boolean operations produced a watertight, manufacturable mesh? Tools: Manifold, OpenSCAD's manifold engine, trimesh `is_watertight` — what's reliable?
- **STEP vs STL vs 3MF:** What format should DesignGenAgent output for maximum downstream compatibility? Are there cases where 3MF is better than STL for production slicing?

---

### Section 9 — Scaling: From One Printer to a Farm

The business goal is to grow. What does multi-printer infrastructure look like?

**Research questions:**
- **Moonraker + multi-printer management:** Can one Moonraker instance manage multiple Klipper instances? What's the architecture for a 2–4 printer farm all reporting to Djinn?
- **Mainsail vs Fluidd vs Obico as multi-printer dashboard:** What's the current best UI for managing a small printer farm (2–5 printers)?
- **Automatic job routing:** For a farm of 2–4 printers, what logic routes a new job to the right printer? (Material loaded, bed temp already at target, no active job, lowest queue depth.) Are there existing tools for this or does Djinn need to build it?
- **Second printer recommendation:** If Typhon's Forge adds a second printer, what's the best choice in 2026 for a high-reliability commission shop running PETG and PLA? Options: another Ender-3 V3 Plus (part compatibility), Bambu X1C (AMS, fast, enclosed, AI monitoring), Prusa MK4 (reliability reputation), Voron 2.4 (community, quality, high investment). Trade-offs for each.
- **Bambu AMS for multi-color/multi-material:** What does AMS unlock for commission products? Cost of entry, reliability in production use, Klipper integration (if any), and whether it's compatible with Djinn's OrcaSlicer pipeline.
- **Queue management at scale:** When running 3+ printers, how does the commission queue model change? Parallel order fulfillment, material reservation, delivery time estimation — what does the Djinn shop system need to add?

---

### Section 10 — Commission Business: Pricing, Positioning, and Growth

The FairPrintAgent covers pricing math. Research the business strategy layer.

**Research questions:**
- **Etsy 3D printing shop landscape in 2026:** Who are the top sellers in the Puffco/dab accessory niche? What are they charging, what materials do they use, what do their listings look like? What's missing from their offering that Typhon's Forge could differentiate on?
- **Puffco accessory market specifically:** What Puffco models are currently most popular (Peak Pro, Proxy, Peak Pro 3D Chamber)? What accessories are highest demand — recyclers, bubblers, attachments, travel cases? Any custom attachment designs that sell consistently? Market size estimate?
- **Cannabis accessory regulations on Etsy:** What are Etsy's current policies on 3D printed smoking accessories (pipes, bubblers, rig attachments) in 2026? Any recent enforcement actions? What terminology and listing practices keep shops compliant vs flagged?
- **Alternative marketplaces:** Beyond Etsy — what other platforms work for 3D printed smoking accessories? (MyCNAmart, Whop, direct Shopify, Faire, specialized cannabis marketplaces?) What are the fees, customer demographics, and enforcement differences?
- **Commission shop workflow best practices:** For a one-person print shop running 10–30 orders/month, what order management, customer communication, and quality assurance practices do successful shops use? Turnaround time expectations, revision policies, deposit requirements?
- **Custom order upsells:** What add-ons consistently increase average order value in 3D print commission shops? (Expedited printing, custom color matching, branded packaging, matching sets, bulk pricing, loyalty pricing for repeat customers?)
- **Print-on-demand model:** Platforms like Printify or Gelato — do they support custom 3D prints? Could Typhon's Forge list designs there for passive income without operating the printer? Or is the custom/functional nature of the products incompatible?

---

### Section 11 — The Original Design Pipeline: Puffco Geometry

The shop's highest-value products are Puffco accessories. These require precise functional geometry — water paths, male/female connections to Puffco parts, heat-safe functional tolerances.

**Research questions:**
- **Puffco Proxy attachment interface:** What are the documented dimensions of the Puffco Proxy male joint (diameter, taper, thread if any)? Are there community-published reverse-engineered drawings or OpenSCAD templates for Proxy-compatible attachments?
- **Puffco Peak Pro attachment interface:** Same question — documented or reverse-engineered dimensions for Peak Pro female bucket/male attachment interface
- **Water recycler geometry:** For a functional recycler (water path that goes into water and recycles vapor through the water), what are the functional design constraints? Path diameter minimums, downstem depth, chamber volume, percolation hole size — any published FDM-printable recycler design guides?
- **Leak-proof FDM for water pieces:** Can FDM PETG print watertight without any coating? What infill density and wall count is required? Any community data on leak testing FDM water pieces?
- **Male joint tolerance for FDM:** For a friction-fit joint between a 3D printed attachment and a glass Puffco joint, what dimensional tolerance in OrcaSlicer achieves a reliable fit (not too loose, not too tight) in PETG? Is this the same tolerance as PLA or does PETG's thermal expansion change it?
- **Heat-resistant materials for dab accessories:** PETG HDT is ~80°C. Puffco attachments get warm but probably not above 80°C on the outer body. Is PETG safe? Any community reports of PETG Puffco accessories deforming in use? What's the actual use-case temperature?

---

### Section 12 — Maker's Mark + Branding System

Every print gets the TF anvil maker's mark engraved on the bottom. Research expanding this.

**Research questions:**
- **Maker's mark standards in FDM:** What's the community standard for maker's mark placement, depth, and size for 3D printed products? Any guidelines from established shops or maker communities?
- **Embossed vs debossed vs color insert:** For a professional product, is bottom deboss (current approach) the best mark placement? What about a raised mark on a visible surface? What do Etsy's top-selling print shops do?
- **Multi-material color insert for branding:** Using a filament change at a specific layer to print the maker's mark in a contrasting color (e.g., black PETG body, white PLA mark at layer 2). Is this reliable on a single-extruder printer via M600 filament change macro? Community success rate?
- **Engraving depth and resolution limits for FDM:** What is the minimum feature size reliably printable at 0.4mm nozzle? The TF anvil has fine detail — what's the smallest line width that reads clearly at production quality?
- **NFC/QR code integration:** Some premium 3D print shops embed a QR code or NFC tag in prints for authentication/provenance. Is this relevant for Typhon's Forge? What's the workflow for embedding a small NFC chip in a print mid-job?
- **Logo placement automation:** For future DesignGenAgent — algorithm for placing a maker's mark correctly on any arbitrary model (largest flat face detection, orientation correction for bottom placement). What's the standard approach?

---

### Section 13 — Klipper Moonraker Integration: What Djinn Isn't Using Yet

Moonraker has a full API. Djinn uses basic endpoints. Research what's available.

**Research questions:**
- **Full Moonraker API surface in 2026:** What endpoints beyond `/server/files/upload` and `/printer/print/start` are useful for an automated pipeline? (Job history, print stats, temperature history, extruder position, layer number, thumbnail access, webcam snapshot?)
- **Klipper virtual sensors:** Can Klipper expose filament usage, layer time, and estimated completion via Moonraker for real-time reporting to Telegram?
- **Moonraker database API:** Klipper persists configuration and history in a local SQLite. Can Moonraker API expose job history (previous prints, time, filament used) without reading SQLite directly?
- **Moonraker webhooks / event notifications:** Does Moonraker support pushing events (print started, layer N reached, error, print complete) to a webhook URL? If so, can Djinn replace its polling monitor with an event-driven model?
- **Power control via Klipper:** Can Moonraker control a smart plug (TPLink/Tasmota) to power-cycle the printer remotely? What's the setup?
- **Spoolman integration:** Spoolman is an open-source filament inventory system that integrates directly with Klipper/Moonraker. In 2026, is it stable enough to replace Djinn's custom inventory tracking? What does it expose via API? Can it replace `djinn-print-quote`'s filament deduction logic?

---

### Section 14 — Business Infrastructure: What the Shop Needs to Operate Legally

Research compliance, business structure, and operational requirements for selling 3D printed products.

**Research questions:**
- **Business structure for a one-person 3D print shop in California in 2026:** Sole proprietor vs LLC. Liability exposure for functional products (Puffco accessories that contact heat). When does the liability risk justify the LLC cost?
- **Product liability for 3D printed functional items:** What is the liability exposure for selling a 3D printed Puffco accessory that fails in use? Does PETG material choice affect this? What product liability insurance exists for small makers?
- **Cannabis accessory sales compliance in California:** Is selling 3D printed dab accessories (non-consumption, accessory only) regulated in CA? Any license requirements? Prop 64 implications?
- **Sales tax nexus for Etsy sellers in California:** What are the current CA sales tax rules for a CA-based Etsy seller? Does Etsy collect and remit, or does the seller need to? Threshold for nexus in other states?
- **USPS shipping policy for cannabis accessories:** USPS has restrictions on cannabis and paraphernalia. Does a 3D printed Puffco attachment qualify as "paraphernalia" under USPS rules? What do other small shops do? UPS/FedEx policies on the same items?
- **Resale certificate for filament purchases:** Can a CA-based print shop get a resale certificate to purchase filament tax-free for use in products sold to customers? How does this work for a sole proprietor?
